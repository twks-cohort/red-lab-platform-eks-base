...
cat kubeconfig_$CLUSTER | opw write red-platform-${CLUSTER} kubeconfig -
...
terraform output cluster_endpoint | tr -d \\n | sed 's/"//g' | opw write red-platform-${CLUSTER} cluster-endpoint -
terraform output cluster_certificate_authority_data | tr -d \\n | sed 's/"//g' | opw write red-platform-${CLUSTER} base64-certificate-authority-data -
